...
struct CostEstimatorView: View {
    ...
    let preview: BatchStagingManager.CostPreview
        ...
        HStack {
            VStack(alignment: .leading) {
                if stagedInputCount == 0 {
                    Text("\(generationCount) generated output\(generationCount == 1 ? "" : "s") @ \(preview.sizeDescription)")
                        .font(.subheadline)
                } else if isMultiInput {
                    Text("\(stagedInputCount) inputs x \(generationCount) generation\(generationCount == 1 ? "" : "s") -> \(preview.outputCount) output\(preview.outputCount == 1 ? "" : "s") @ \(preview.sizeDescription)")
                        .font(.subheadline)
                } else {
                    Text("\(stagedInputCount) inputs x \(generationCount) generation\(generationCount == 1 ? "" : "s") -> \(preview.outputCount) output\(preview.outputCount == 1 ? "" : "s") @ \(preview.sizeDescription)")
                        .font(.subheadline)
                ...
                if preview.inputCost > 0 {
                    let inputRate = preview.inputCost / Double(max(1, preview.inputCount))
                    let outputRate = preview.outputCost / Double(max(1, preview.outputCount))
            ...
            Spacer()
            Text("≈ $\(preview.total, specifier: "%.2f")")
                .font(.headline)
